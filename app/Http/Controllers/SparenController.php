...
class SparenController extends Controller
{
    ...
    public function persistTransactions(Request $request): JsonResponse
    {
        $data = $request->validate([
            ...
            'items.*.allocations.*.budgetItemId' => ['required_with:items.*.allocations', 'string'],
            'items.*.allocations.*.amount' => ['required_with:items.*.allocations', 'numeric'],
    ...
    public function persistTransaction(Request $request, string $txId): JsonResponse
    {
        $data = $request->validate([
            ...
            'allocations.*.budgetItemId' => ['required_with:allocations', 'string'],
            'allocations.*.amount' => ['required_with:allocations', 'numeric'],
