...
const deleteBudgetPayload = ref<{ categoryId: number; budgetId: number; budgetName: string; transactionsCount: number } | null>(null);
...
const openDeleteBudgetModal = (categoryId: number, budget: Record<string, any>) => {
    deleteBudgetPayload.value = {
        categoryId,
        budgetId: Number(budget.id),
        budgetName: String(budget.name ?? 'Onbekend budget'),
        transactionsCount: Number(budget.payments ?? 0),
    };
};

const deleteBudget = () => {
    if (!deleteBudgetPayload.value || deletingBudget.value) {
        return;
    }

    deletingBudget.value = true;
    const { categoryId, budgetId } = deleteBudgetPayload.value;

    router.delete(`/categories/${categoryId}/budgets/${budgetId}`, {
        preserveScroll: true,
        onSuccess: () => {
            deleteBudgetPayload.value = null;
            showSuccess('Budget verwijderd en transacties ontkoppeld.');
            router.reload({
        ...
        <DialogContent class="sm:max-w-lg">
            ...
            <div class="space-y-4">
                ...
                <Form
                    ...
                    @start="isUpdatingCategory = true"
                    @finish="isUpdatingCategory = false"
                    @success="
                        () => {
                            showSuccess('Categorie opgeslagen.');
                    ...
                    <div class="space-y-3 sm:col-span-2 border-t pt-3">
                        ...
                        >
                            ...
                            <div class="col-span-1 flex items-center justify-end">
                                <button v-if="budget.id && props.category" type="button" class="flex h-9 w-9 items-center justify-center rounded-md text-red-400 hover:bg-red-950/40" @click="openDeleteBudgetModal(props.category.id, budget)">
            ...
            <Dialog :open="deleteBudgetPayload !== null" @update:open="(open) => { if (!open) deleteBudgetPayload = null; }">
                <DialogContent class="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Budget <strong>{{ deleteBudgetPayload?.budgetName }}</strong> heeft
                            <strong>{{ deleteBudgetPayload?.transactionsCount ?? 0 }}</strong> gekoppelde transacties.
                            Deze transacties worden ontkoppeld. Weet je zeker dat je wilt verwijderen?
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        <Button type="button" variant="secondary" :disabled="deletingBudget" @click="deleteBudgetPayload = null">Annuleren</Button>
                        <Button type="button" variant="destructive" :disabled="deletingBudget" @click="deleteBudget">
                            {{ deletingBudget ? 'Verwijderen...' : 'Verwijderen' }}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <DialogFooter class="mt-4 gap-2">
                ...
                <Button form="category-update-form" type="submit"
                    ...
                    >{{ isUpdatingCategory ? 'Opslaan...' : 'Opslaan' }}</Button
                >
            </DialogFooter>
        </DialogContent>
    </Dialog>
