...
import { home, income } from '@/routes';
...
const applyMonth = () => {
    ...
    router.get(`/income/${slug}`, { month: month.value }, { preserveState: false, preserveScroll: true });
};
            ...
            <div :class="`${customScrollbar} lg:hidden overflow-x-auto mb-1`">
                <div class="flex gap-4">
                    <template v-for="budget in categories" :key="budget.id">
                        ...
                        <Link :href="`/income/${budget.slug}?month=${month}`" as="div">
